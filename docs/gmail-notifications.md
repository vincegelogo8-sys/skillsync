...
- Admin uses **Approve & Assign** in `AdviserAssignmentService::approve`; the request becomes `approved` and its `studentProfile.user` receives `AdviserRequestAcceptedNotification`. Assignment creation and cancellation of competing pending requests remain in the same transaction.
- Admin or the requested Faculty member declines through `AdviserRequestService::close`; the request becomes `declined` and its `studentProfile.user` receives `AdviserRequestDeclinedNotification`.
...
5. Either log in as Admin and choose **Approve & Assign**, or decline as Admin/the requested Faculty member. Faculty cannot approve.
