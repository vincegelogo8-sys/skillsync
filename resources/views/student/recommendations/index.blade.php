<x-app-layout>
    <x-slot name="header"><h2 class="font-semibold text-xl text-gray-800">{{ __('Adviser Recommendations') }}</h2></x-slot>
    <div class="py-12"><div class="mx-auto max-w-7xl space-y-6 sm:px-6 lg:px-8">
        <section class="bg-white p-4 shadow sm:rounded-lg sm:p-8">
            <a href="{{ route(($admin ? 'admin' : 'student').'.proposals.show', $proposal) }}" class="text-sm text-indigo-600 underline">{{ __('Back to proposal') }}</a>
            ...
            @if (session('status') === 'recommendations-generated')<p role="status" class="mt-4 text-sm font-medium text-green-700">{{ __('Recommendations generated successfully.') }}</p>@endif
            @if (session('status') === 'recommendations-empty')<p role="status" class="mt-4 text-sm text-gray-700">{{ __('No faculty profiles are available to rank.') }}</p>@endif
