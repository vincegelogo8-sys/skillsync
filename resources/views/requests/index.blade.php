<x-app-layout>
    ...
    <div class="py-12"><section class="mx-auto max-w-7xl bg-white p-6 shadow sm:rounded-lg">
        ...
        @if ($role === 'admin')<p class="mt-3 text-sm text-gray-600">{{ __('Approve & Assign creates the final active assignment and cancels other pending requests for that proposal. Capacity is checked again when you approve.') }}</p>@endif
                ...
                <article class="py-5">
                    ...
                    <div class="mt-4 flex flex-wrap items-center gap-4">
                        ...
                        @if ($adviserRequest->status === 'pending')
                            @if ($role === 'admin')
                                <form method="POST" action="{{ route('admin.requests.approve', $adviserRequest) }}">
                                    @csrf
                                    <x-primary-button>{{ __('Approve & Assign') }}</x-primary-button>
                                </form>
                            @endif
