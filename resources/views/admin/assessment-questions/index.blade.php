<x-app-layout>
    <x-slot name="header"><h1 class="font-semibold text-xl text-gray-800">{{ __('Skills Assessment Questions') }}</h1></x-slot>
    <x-slot name="description">{{ __('Maintain the question bank used for faculty skills assessments.') }}</x-slot>
    <div class="py-12"><div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <section class="bg-white p-4 shadow sm:rounded-lg sm:p-8">
            <div class="flex flex-wrap items-center justify-between gap-4">
                ...
                <a href="{{ route('admin.assessment-questions.create') }}" class=" button button-primary">{{ __('Add Question') }}</a>
            </div>
            ...
            @if (session('status'))<p role="status" class="ui-alert mt-4 text-sm text-green-700">{{ session('status') }}</p>@endif
            <div class="mt-4 divide-y divide-gray-200">
                @forelse ($questions as $question)
                    <article class="py-5 space-y-3">
                        ...
                        <div class="flex flex-wrap items-start gap-4">
                            <a href="{{ route('admin.assessment-questions.edit', $question) }}" class=" button button-secondary">{{ __('Edit question') }} #{{ $question->id }}</a>
                ...
                @empty
                    <x-empty-state title="{{ __('No questions yet. Add questions and their correct answers to prepare the assessment.') }}" />
                @endforelse
            </div>
