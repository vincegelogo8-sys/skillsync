<x-app-layout>
    <x-slot name="header"><h1 class="text-xl font-semibold text-gray-800">{{ $role === 'faculty' ? __('Assigned Students') : __('Adviser Assignments') }}</h1></x-slot>
    <x-slot name="description">{{ __('Review confirmed research adviser assignments.') }}</x-slot>
    <div class="py-12"><section class="mx-auto max-w-7xl bg-white p-6 shadow sm:rounded-lg">
        ...
        @if (session('status') === 'adviser-assigned')<p role="status" class="ui-alert mt-4 text-sm text-green-700">{{ __('Adviser assigned successfully.') }}</p>@endif
        <div class="mt-5 divide-y divide-gray-200">
            ...
            @empty
                <x-empty-state title="{{ __('No adviser assignments yet.') }}" />
            @endforelse
        </div>
